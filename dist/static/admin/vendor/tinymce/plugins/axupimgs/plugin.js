				...
				switch (details.name) {
					case 'save':
						...
						for(let i=0;i<len;i++){
							...
							// 有大小尺寸适配，url是个对象
							else if( imgs[i].source.url ){
								html += `
								<picture>
									<source  media="(max-width: 700px)" srcset="${imgs[i].source.min}">
									<source  media="(max-width: 900px)" srcset="${imgs[i].source.max}">
									<img width="100%" src="${imgs[i].source.url}" alt="">
								</picture>
								`
							}
						}
