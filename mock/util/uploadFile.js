 ...
 * LastEditTime : 2020-06-10 19:30:18
 * Description  : 封装文件上传
 */
...
module.exports = (req, res) => {
	...
	form.parse(req, (err, fields, files) => {
		...
		if (file.size > form.maxFieldsSize) {
			fs.unlink(file.path)
			return res.status(200).json({ code: "412", error: "文件不能超过10M" })
		}
		...
		fs.rename(file.path, newPath, (err) => {
			if (err) {
				return res.status(200).json({ code: 412, "error": "文件上传失败" })
			} else {
				return res.send({ 
					"code": 200, 
					"success": "文件上传成功", 
					location: "http://localhost:9999/file/" + fileName
				})
			}
		})
	})
