/*
 * Author       : JackWei <wsm_1105@163.com>
 * LastEditors  : JackWei <wsm_1105@163.com>
 * Date         : 2020-06-03 09:25:08
 * LastEditTime : 2020-06-10 17:30:50
 * Description  : 封装图片上传
 */

const fs = require("fs")
const path = require("path")
const formidable = require("formidable")
const formatTime = require("silly-datetime")

/* 图片上传 */
module.exports = (req, res) => {
	let form = new formidable.IncomingForm()
	form.encoding = "utf-8"
	form.uploadDir = path.join(__dirname, "../static/img")
	form.keepExtensions = true
	form.maxFieldsSize = 5 * 1024 * 1024
	form.parse(req, (err, fields, files) => {
		let file = files.file
		/* 如果出错，则拦截 */
		if (err) {
			return res.status(500).json({ code: "500", error: "服务器内部错误" })
		}
		if (file.size > form.maxFieldsSize) {
			fs.unlink(file.path)
			return res.status(200).json({ code: "412", error: "图片不能超过5M" })
		}

		let extName = ""

		switch (file.type) {
			case "image/png":
				extName = "png"
				break
			case "image/x-png":
				extName = "png"
				break
			case "image/jpg":
				extName = "jpg"
				break
			case "image/jpeg":
				extName = "jpg"
				break
		}
		if (extName.length == 0) {
			return res.status(200).json({ code: 412, error: "只支持jpg和png格式图片" })
		}

		/* 拼接新的文件名 */
		let time = formatTime.format(new Date(), "YYYYMMDDHHmmss")
		let num = Math.floor(Math.random() * 8999 + 10000)
		let imageName = `${time}_${num}.${extName}`
		let newPath = form.uploadDir + "/" + imageName

		fs.rename(file.path, newPath, (err) => {
			if (err) {
				return res.status(200).json({ code: 412, "error": "图片上传失败" })
			} else {
				return res.send({ 
					"code": 200, 
					"success": "图片上传成功", 
					location: "http://localhost:9999/img/" + imageName,
					min: "http://localhost:9999/img/20200610154503_11499.jpg",
					max: "http://localhost:9999/img/20200610150112_14960.jpg" 
				})
			}
		})
	})
}
